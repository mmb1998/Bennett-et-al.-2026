...
function [test_freqs,test_pv,pv]=measure_calibrated_pv(pv_calib_filename,test_freqs,test_pv,temp,humid,press)
...
nidaq_ai_channel = string(setup_data{"nidaq_ai_channel", 2}); %read pressure mic input; not used but needed for CalibPlay
nidaq_ao_channel = string(setup_data{"nidaq_ao_channel", 2}); %sound generation

%calibrated pv-specific code
pv_per_v_mult = spk_freq_resp_pv(test_freqs); 
if any(max(test_pv)./pv_per_v_mult>1)
...
if(sum((test_pv>120)|(test_pv<0))>0)
...
%test_pv=[120 test_pv]; %if attens is 120, play no sound

disp(['estimated time = ' num2str(nreps*length(test_freqs)*length(test_pv)*2*(pre+len+post)/60) ' min']);

time=clock;

%ignore in1
in1=zeros(length(test_pv),length(test_freqs),nreps,round(pre*Fs)+round(len*Fs)+round(post*Fs));
in2=zeros(length(test_pv),length(test_freqs),nreps,round(pre*pv_usb_samp_rate)+round(len*pv_usb_samp_rate)+round(post*pv_usb_samp_rate));
...
for(j=1:length(test_freqs))
    ...
    for(k=1:nreps)
        disp(['rep=' num2str(k)]);
        i=1;
        while(i<=length(test_pv))
            atten = test_pv(i)/pv_per_v_mult(j); %this is the voltage multiplier
            atten = 10*log10(1/atten); % mult = (1/(10^(atten/10))) %note issue with this formula but keep consistent with CalibPlay for now
            if(atten==120) %if attens is 120, play no sound
                %[in1(i,j,k,:) in2(i,j,k,:)]=calibrate_play(foo,out_idx,freqs(j),attens(i),'L',highpass,lowpass,mic_clip);
                [in1(i,j,k,:) in2(i,j,k,:)]=CalibPlay(foo,out_idx,test_freqs(j),atten,'L',highpass,lowpass,mic_clip,out_idx2);
            else
                [in1(i,j,k,:) in2(i,j,k,:)]=CalibPlay(out,out_idx,test_freqs(j),atten,'L',highpass,lowpass,mic_clip,out_idx2);
            end
...
for(j=1:length(test_freqs))
    if test_freqs(j) > 100
        for k = 1:nreps
            for(i=1:length(test_pv))
                if max(in2(i, j, k, out_idx2)) <= 0.8 && max(in2(i, j, k, out_idx2)) >= 0.02
...
pv=zeros(length(test_pv),length(test_freqs));
for(j=1:length(test_freqs))
  idx=find(snr2(1:end,j)>snr_crit); %find indices for all non-zero attens that are above snr_crit
  if(length(idx)>1)
    pv(:,j)= amp2(idx,j)/interp1(freqs, So2(:,1), test_freqs(j), 'makima', 'extrap'); %divide amplitude by mic sensitivity to get pv in m/s
  elseif(length(idx)==1)
    pv(1,j)= amp2(idx,j)/interp1(freqs, So2(:,1), test_freqs(j), 'makima', 'extrap'); %divide amplitude by mic sensitivity to get pv in m/s
  else
    pv(1,j)=nan;
  end
end
