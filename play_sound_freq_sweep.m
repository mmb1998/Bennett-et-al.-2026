function play_sound_freq_sweep(stimulus,delay_enable)
global Fs;
Fs = 16000; %Hz
...
nidaq_do_channel_start = "port0/line0"; %imaging acquisition start trigger
nidaq_do_channel_stimulus = "port0/line1"; %stimulus (e.g. red light for CsChrimson)
nidaq_samp_rate = Fs;
...
ramp=0.01; %was 0.05
pre=20; %was 5
len=10; %duration of sound delivery, was 0.5
post=15; %was 0.1
nreps=1;
...
%up-chirp linear
%out=10.*(sin(2*pi*freqs*(1:round(len*Fs))/Fs).*linspace(0,1,round(len*Fs)));
out=10.*my_env(chirp((1:round(len*Fs))/Fs, freqs(1), round(len*Fs)/Fs, freqs(end)) ,'cosine',ramp,ramp,Fs);
%max of 10 volts
...
if stimulus == true
    ...
    if (stim_idx + size(stimulus_data,2)) > out_idx
        stimulus_data = [stimulus_data, ones(1, out_idx - size(stimulus_data,2) - 1), 0];
    else
        stimulus_data = [stimulus_data, ones(1,stim_idx), zeros(1,(out_idx - stim_idx - size(stimulus_data,2)))];
    end
end
