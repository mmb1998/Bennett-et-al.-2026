...
%first calculate 'correct' pv_usb_sample rate from highest amplitude and highest frequency recordings,
%since it deviates slightly from expected
est_pv_usb_sample_rate = zeros(nreps, 1);
for k = 1:nreps
    [~, ~, est_pv_usb_sample_rate(k)] = fit_sin_approx(squeeze(in2(2, max([1, end-1]), k, out_idx2)), test_freqs(max([1, end-1])), pv_usb_samp_rate);
end
